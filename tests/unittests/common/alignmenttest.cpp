...
TEST_P(AlignmentTest, testConstructFromSExpression) {
  ...
  SExpression node = SExpression::parse(data.serialized, FilePath());

  if (data.validSExpression) {
    EXPECT_EQ(Alignment(data.hAlign, data.vAlign), Alignment(node));
  } else {
    EXPECT_THROW({ Alignment a(node); }, RuntimeError);
  }
}
...
TEST_P(AlignmentTest, testSerialize) {
  ...
  if (data.validSExpression) {
    ...
    SExpression node = alignment.serializeToDomElement("align");
    EXPECT_EQ(data.serialized.toStdString(), node.toByteArray().toStdString());
  }
}
